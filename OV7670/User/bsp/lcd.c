...
//LCD_Reg:寄存器地址
//LCD_RegValue:要写入的数据
void LCD_WriteReg(uint8_t LCD_Reg, u16 LCD_RegValue)
{	
...
{ 										  
 ...
 //	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID  
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		sw_delay_ms(200); // Dis-charge capacitor power voltage
		LCD_WriteReg(0x0010, 0x1690); // SAP, BT[3:0], AP, DSTB, SLP, STB
		LCD_WriteReg(0x0011, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
		sw_delay_ms(50); // Delay 50ms
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,uint8_t size,uint8_t *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
